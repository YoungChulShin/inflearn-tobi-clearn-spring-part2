...
#### 행위
- `static register()`: 회원 등록. `MemberRegisterInfo`, `PasswordEncoder`를 받는다
- `activate()`: 등록을 완료시킨다
- `deactivate()`: 탈퇴시킨다
- `verifyPassword()` : 비밀번호를 검증한다
- `changePassword()`
- `updateInfo()`: 회원 정보 수정. 회원의 닉네임, 프로필 주소, 자기 소개를 수정하다
- `isActive()`: 등록 완료 상태인지 확인
#### 규칙
- 회원 생성후 상태는 등록 대기
...
#### 속성
- `id`: `Long`
- `profile`: `Profile` 프로필 주소. 모든 회원이 고유한 프로필 주소를 가져야 한다
- `introduction`: `String` 자기 소개
- `registeredAt`: `LocalDateTime` 등록 일시
- `activatedAt`: `LocalDateTime` 등록 완료 일시
...
- `updateInfo()`: 상세 정보 수정. 프로필 주소가 빈 값이면 `null`로 저장(프로필 제거)
#### 규칙
- `activatedAt`, `deactivatedAt`은 한 번만 설정할 수 있다
- 생성/수정 메서드는 패키지 접근 수준. `Member`를 통해서만 변경한다
...
_Value Object_
#### 속성
- `address`: `String` 프로필 주소
#### 행위
- `url()`: `@` + 주소 형태의 URL 반환
#### 규칙
- 소문자 알파벳과 숫자만 허용, 최대 15자
- 빈 문자열 허용(프로필 주소 제거용). `null`은 불허
- 형식 위반 시 `IllegalArgumentException`
...
#### 규칙
- 이메일 정규식 위반 시 `IllegalArgumentException`
---
